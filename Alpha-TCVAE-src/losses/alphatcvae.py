...
class Loss(dent.losses.baseloss.BaseLoss):
    ...
    def __call__(self, data, reconstructions, stats_qzx, is_train, samples_qzx, **kwargs):
        ...
        KL_CEB = (log_qzx - log_qz.unsqueeze(-1)).mean()
        ...
        KL_VIB = (log_qz_product - log_pz).mean()

        # total loss
        loss = rec_loss + (K*self.alpha)/(L - self.alpha) * KL_CEB - (1 - self.alpha)/(1 - self.alpha/K) * KL_VIB
